...
final class ConfigTest: XCTestCase {
    ...
    func testParseMode() {
        ...
        assertEquals(errors, [])
        let binding = HotkeyBinding(.option, .h, [FocusCommand.new(cardinalOrDfsDirection: .cardinal(.left))])
        assertEquals(
    ...
    func testHotkeyParseError() {
        ...
        let binding = HotkeyBinding(.option, .k, [FocusCommand.new(cardinalOrDfsDirection: .cardinal(.up))])
        assertEquals(
            config.modes[mainModeId],
            Mode(name: nil, bindings: [binding.descriptionWithKeyCode: binding])
        )
